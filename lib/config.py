...
def field(index: int, placeholder: str, **kwargs: Any) -> str:
    "Produces tabstops for editor"
    if placeholder:
        return f"${{{index:d}:{placeholder}}}"
    return f"${{{index:d}}}"


def field_preview(index: int, placeholder: str, **kwargs: Any) -> str:
    "Produces tabstops for abbreviation preview"
    return placeholder


def get_config(view: sublime.View, pos: int, params: dict[str, Any] | None = None) -> Config:
...
def get_output_options(view: sublime.View, inline: bool = False) -> dict[str, Any]:
