...
from ..emmet.css_matcher import TokenType
from ..emmet.css_matcher import scan as scan_css
from ..emmet.html_matcher import attributes
from ..emmet.stylesheet import CSSAbbreviationScope
from . import syntax
from .config import get_config
from .utils import attribute_value, get_content
...
def get_activation_context(editor: sublime.View, pos: int) -> Config | None:
    """
    ...
    if syntax.is_css(syntax_name):
        ...
        if ctx:
            ...
            if editor.match_selector(
                pos, "meta.at-rule.media meta.group | meta.at-rule.supports meta.group"
            ):
...
def create_activation_context(
    editor: sublime.View, pos: int, context: dict[str, Any] | None = None, inline: bool = False
) -> Config:
...
def get_html_context(editor: sublime.View, pos: int) -> dict[str, Any] | None:
    """
                ...
                if is_close:
                    ...
                    if is_self_close:
                        if region_contains(r, pos):
                            return create_tag_context(editor, tag_name, r)
                    else:
...
def fast_get_css_context(editor: sublime.View, pos: int) -> dict[str, Any] | None:
    ...
    if r:
        return get_css_context_from_text(text, pos - r.a)
    return None
...
def search_css_context(content: str, pos: int) -> dict[str, Any]:
    ...
    state: dict[str, Any] = {"current": None, "pool": [], "stack": []}

    def scan_callback(token_type: str, start: int, end: int, delimiter: int) -> bool | None:
        if start >= pos:
        ...
        if start < pos <= end:
            # Direct hit on token
            state["current"] = {"type": token_type, "region": sublime.Region(start, end)}
            return False
...
def get_css_context_from_text(text: str, pos: int) -> dict[str, Any] | None:
    ...
    ) or is_typing_before_selector(text, pos, cur):
...
def get_css_context(editor: sublime.View, pos: int) -> dict[str, Any] | None:
...
def is_typing_before_selector(text: str, pos: int, ctx: dict[str, Any] | None) -> bool:
    """
...
def alloc_item(pool: list[dict[str, Any]], name: str, region: sublime.Region) -> dict[str, Any]:
...
def release_item(pool: list[dict[str, Any]], item: dict[str, Any]) -> None:
    pool.append(item)


def pop_tag_stack(
    stack: list[dict[str, Any]], tag: str, pool: list[dict[str, Any]]
) -> dict[str, Any] | None:
    while stack:
        item = stack.pop()
...
def alloc_css_item(
    pool: list[dict[str, Any]], token_type: str, start: int, end: int
) -> dict[str, Any]:
    if pool:
        item = pool.pop()
...
def release_css_item(pool: list[dict[str, Any]], item: dict[str, Any] | None) -> None:
    if item is not None:
        pool.append(item)


def parse_html_attributes(
    editor: sublime.View, name: str, open_tag: sublime.Region
) -> dict[str, str]:
...
def create_tag_context(editor: sublime.View, name: str, open_tag: sublime.Region) -> dict[str, Any]:
    "Factory method for creating tag context"
    return {"name": name, "attributes": parse_html_attributes(editor, name, open_tag)}
