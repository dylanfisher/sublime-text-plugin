...
from . import syntax
from .utils import get_caret, get_content

html_comment = {"start": "<!--", "end": "-->"}

css_comment = {"start": "/*", "end": "*/"}

comment_selector = "comment"
embedded_style = (
    "source.css.embedded | source.less.embedded | source.scss.embedded"
    " | source.sass.embedded | source.sss.embedded"
)


def remove_comments(
    view: sublime.View, edit: sublime.Edit, region: sublime.Region, tokens: dict[str, str]
) -> int:
    ...
    if text.startswith(tokens["start"]) and text.endswith(tokens["end"]):
        ...
        end_region = sublime.Region(
            end_region.begin() - start_region.size(), end_region.end() - start_region.size()
        )
...
def get_range_for_comment(view: sublime.View, pt: int) -> sublime.Region | None:
...
def get_comment_regions(
    view: sublime.View, region: sublime.Region, tokens: dict[str, str]
) -> list[sublime.Region]:
