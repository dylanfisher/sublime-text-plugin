...
pairs_end = {v: k for k, v in pairs.items()}
...
def narrow_to_non_space(
    view: sublime.View, region: sublime.Region, direction: int = NON_SPACE_LEFT | NON_SPACE_RIGHT
) -> sublime.Region:
...
def replace_with_snippet(
    view: sublime.View, edit: sublime.Edit, region: sublime.Region, snippet: str
) -> None:
    ...
    view.run_command("insert_snippet", {"contents": preprocess_snippet(snippet)})


def multicursor_replace_with_snippet(
    view: sublime.View, edit: sublime.Edit, payload: Iterable[tuple[sublime.Region, str]]
) -> None:
...
def go_to_pos(view: sublime.View, pos: int) -> None:
...
def is_url(file_path: str) -> bool:
    ...
    return re.match(r"^\w+?://", file_path) is not None


def read_file(file_path: str, size: int = -1) -> bytes:
    "Reads content of given file. If `size` if given, reads up to `size` bytes"
    if is_url(file_path):
        with urllib.request.urlopen(file_path, timeout=5) as req:  # noqa: S310 - user's own URL
...
def locate_file(editor_file: str, file_name: str) -> str | None:
    """
...
def create_path(parent: str, file_name: str) -> str:
    """
...
def attribute_value(attr: AttributeToken) -> str:
...
def patch_attribute(attr: AttributeToken, value: str, name: str | None = None) -> str:
    ...
    return f"{name}={before}{value}{after}"


def patch_property(view: sublime.View, prop: Any, value: str, name: str | None = None) -> str:
    """Returns patched version of given CSS property, parsed by Emmet CSS matcher.
    `prop` is a CSSProperty whose ranges were converted to regions (css_section())."""
    if name is None:
...
def is_quoted(value: str | None) -> bool:
    "Check if given value is either quoted or written as expression"
    if not value:
        return False
    return (value[0] in "\"'" and value[0] == value[-1]) or (value[0] == "{" and value[-1] == "}")


def attribute_region(attr: AttributeToken) -> sublime.Region:
...
def to_region(rng: Sequence[int]) -> sublime.Region:
